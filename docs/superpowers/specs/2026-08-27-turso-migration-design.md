...
読み取り・書き込みが失敗した場合の挙動。要件の「読めなければストリークなしで通知し、書けなければその日の確定をあきらめる」に従う。

**朝通知の読み取り失敗は、未初期化と同じく確定処理をスキップする。** 当初この設計は「既存挙動のまま（空状態で続行して次回保存で自己修復）」としていたが、それは誤りだった。`morning-index.js` の既存コードは読み込み失敗時に `previousStreakUsers = {}` へフォールバックしたうえで確定処理を回し、`saveStreakData` で**書き戻す**。データがローカルファイルにあった時代は失敗が「壊れた JSON」か「未知バージョン」だけで稀であり、次回保存での自己修復が成立した。Turso に移すと同じ経路が一過性のネットワーク障害でも通り、全ユーザーの `streak` / `grace` / `bonus` を空状態で上書きして恒久的に失う。`bonus` は実際に現金で支給する値で復元手段がない。未判定の日は中立扱い（ペナルティなし）なので、スキップしても子供は損をしない。

| エントリ | 読み取り失敗 | 書き込み失敗 |
|---|---|---|
| 夜通知 | 警告のみで続行（免除日なし扱い） | `errors` に積んで終了コード1 |
| 朝通知 | **確定処理をスキップ**し、ストリーク行なしで通知を出す。`errors` に積んで終了コード1 | `errors` に積んで終了コード1 |
