...
夜通知は「当日のストリーク要件未達のユーザーが1人でもいる日」だけLINEに送る（`src/index.js` の `hasUnqualifiedUser`）。土日のしきい値が下がることで、中学生が土日に3〜4講座を終えた日はこれまで未達扱いだったのが達成扱いになるため、**LINEの送信回数はわずかに減る方向**に動く。月間200カウントの枠に対して有利な方向であり、枠を圧迫する変更ではない。
