...
- 夜通知はユーザー名だけを見出しごとに並べる: 未達は `🚨 まだ今日の学習が終わっていません`、達成は `✅ 今日の学習が終わりました`、免除日は `🏝️ 今日はおやすみ（免除日）`（免除日は未達にも達成にも数えない）。未達も取得失敗もいない日の達成の見出しは `✅ 全員が本日の学習を終えました`（免除日のユーザーがいる日は `✅ おやすみの人以外は本日の学習を終えました`）に変わる。該当者がいない見出しは出さない
