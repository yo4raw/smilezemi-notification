...
1. **日次通知** (`src/index.js`): 毎日 JST 20:00 に実行。両コース(小学生・中学生)の当日分を速報通知。**本文は当日のストリーク要件が未達のユーザー名だけを並べる**（`formatUnqualifiedMessage`。学習件数・ミッション詳細・勉強時間・ストリーク・おたすけ・ボーナスはいずれも出さず、翌朝の確定通知がカバーする）。`streak_data.json` は免除日の判定にだけ読む。**LINE送信数節約のため、未達のユーザーが1人でもいる日だけLINEに送信**（全員達成日はLINEに送らず、断り行を付けてDiscordのみに記録する）
...
- 夜通知は未達ユーザー名を `🚨 まだ今日のノルマが終わっていません` の下に `👤 名前` で並べるだけにする。免除日のユーザーは `🏝️ 今日はおやすみ（免除日）` の下に分けて並べ、未達には数えない。未達も取得失敗もいない日は `✅ 全員が本日のノルマを達成しました`（免除日のユーザーがいる日は `✅ おやすみの人以外は本日のノルマを達成しました`）の1行になる
