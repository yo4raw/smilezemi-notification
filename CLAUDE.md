...
- 学習判定はコースで異なる: **小学生コースは「完了ミッション5個以上」が必須**（勉強時間は見ない。夜通知が `minCompletedMissions: 5` を指定）。中学生コースは「勉強時間0 かつ ミッション0件」= 未学習の現行基準。学習した日は `streak += 1`、連続10日ごとに「おたすけ」+1（上限3）。**初期おたすけは1**（初回特典。`streak_data.json` v1.0→v1.1移行で既存ユーザーも最低1に引き上げ）。streak 0 のときは消費せず、リセット後は0から再スタート
