---
name: smilezemi-set-grace
description: スマイルゼミ通知システムの「おたすけ(grace)」ポイントを手動で変更するときに使う。子どものおたすけ残数を特定の値に設定したい、おたすけを補充・追加したい、減らしたい、grace を調整したいといった依頼で必ずこのスキルを使うこと。連続学習日数(streak)の変更は smilezemi-set-streak、ボーナスポイント(bonus)の変更は smilezemi-set-bonus という別スキルなので、それらの依頼では使わない。変更は本番(GitHub Actions キャッシュ)に反映される。
---

# おたすけ(grace)手動変更

スマイルゼミ通知システムの `grace`（おたすけ残数）を、指定ユーザーについて**絶対値**で設定する。

なぜ専用スキルなのか: grace / streak / bonus は別々のスキルに分離してある。このスキルは
**`field=grace` しか触らない**。取り違えて連続日数やボーナスを書き換える事故を防ぐためなので、
このスキルで streak や bonus を変更しようとしないこと（それぞれ専用スキルがある）。
...
- 範囲は **0〜3**（`GRACE_MAX=3`）。範囲外はスクリプト側で拒否される。
...
ユーザーに「どの子（どのコース）」の grace を「いくつ」にするか確認する。
grace は 0〜3 の絶対値で指定する（「+1」ではなく「3 にする」の形）。
...
出力の `"<キー>": ... grace=...` から、正確なキーと現在の grace を読み取る。
...
ログの「変更前: grace=X」「変更後: grace=Y」「保存しました」を確認し、ユーザーに報告する。
実際の通知メッセージには次回のスケジュール実行で反映される旨も伝える。

## 失敗時の対応

- 「対象ユーザーが見つかりません」→ 手順2に戻り、候補一覧から正確なキーをコピーし直す。
- 「grace は 0〜3 の範囲で…」→ 指定値を 0〜3 に収める。
