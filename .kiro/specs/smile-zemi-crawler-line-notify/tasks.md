...
- [ ] 9.1 GitHub Actions環境での手動実行テスト
  ...
  - **注記**: 検証チェックリスト（docs/VALIDATION_CHECKLIST.md）に詳細手順を記載

- [ ] 9.2 エラーリカバリーシナリオのテスト
  - ネットワーク一時障害シミュレーション（リトライ動作確認）
  - ログイン失敗時のエラー処理確認
  - DOM要素未検出時のスクリーンショット保存確認
  - LINE通知失敗時のリトライとエラーログ確認
  - _Requirements: 7.1, 7.2, 7.3, 7.4_
  - **注記**: 検証チェックリスト（docs/VALIDATION_CHECKLIST.md）にシナリオを記載
...
- [ ] 9.4 初回cron実行の確認
  ...
  - **注記**: 検証チェックリスト（docs/VALIDATION_CHECKLIST.md）に手順を記載
